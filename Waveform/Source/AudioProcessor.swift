...
extension AudioProcessor {
    ...
    fileprivate func extract(samplesFrom assetReader: AVAssetReader) -> [Float] {
        ...
        var paddedSamples = [Float](repeating: silenceDbThreshold, count: outputSamples.count)
//        paddedSamples.replaceSubrange(0..<min(targetSampleCount, outputSamples.count), with: outputSamples)
    ...
    private func process(_ samples: UnsafeMutablePointer<Int16>,
                         ...
                         from assetReader: AVAssetReader) -> [Float] {
        ...
        let samplesPerPixel = 1// max(1, sampleCount(from: assetReader) / targetSampleCount)
